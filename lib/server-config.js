module.exports = {
    ...
    registerApi: function (app) {
        var config = this;
        var fs = require('fs');
        config.lodash(config.api_path).forEach(function (path) {
    ...
    allowedUserCount: 10,
    isValidSignUpCode: function (signUpCode) {
        var config = this;
        if (config.lodash.contains(config.validCodes, signUpCode)) {
            return true;
        }
        
        return false;
    },
    canUserSignUp: function (userCount) {
        var config = this;
        if (userCount < config.allowedUserCount) {
            return true;
        }
        return false;
    }
