var Config = {
    ...
    useHttps: (function () {
        if (process.env.OPENSHIFT_USE_HTTPS) {
            return true;
        } else {
            return false;
        }
    ...
    auth: {
        ...
        ensureApiAuthenticated: function (req, res, next) {
            if (req.isAuthenticated()) {
                ...
                var err = (req.session.error)? req.session.error: new AppError(
                     'Login Error', 
...
Config.session = (function () {
    ...
    if (Config.useHttps) {
        ...
        sessionOpts.cookie.maxAge = 60 * 1000 * 15;
    }
...
//=============== Session ==================
//=============== Reverse Proxy ============
Config.reverseProxy = function (app){
    if (Config.useHttps) {
